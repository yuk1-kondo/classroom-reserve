...
### 🖥️ UI
- サイドパネルの管理者機能に「🚫 予約禁止期間設定」を追加
- 禁止期間の一覧表示・追加・削除UI
- 開始日/終了日/対象教室/理由の入力フォーム

### 🔧 技術的な変更
- `blocked_periods` Firestoreコレクションを新設
- `blockedPeriodsService` サービス層を実装（CRUD + チェック機能）
- `BlockedPeriodsSettings.tsx` 管理者UIコンポーネントを追加
- 予約作成時に禁止期間バリデーションを追加（一般ユーザーのみ）

### 🔒 Firestoreルール
- `blocked_periods` コレクション: 読み取りは全員、書き込みは管理者のみ

### 🐞 修正
- Firestoreが`undefined`を受け付けない問題を解消（オプションフィールドの適切な除外処理）

---

## [2.7.0] - 2026-01-20 🔒 **会議室パスコード削除の安定化**

### 🐞 修正
- **会議室のパスコード削除が失敗する問題を解消**
  - Firestoreルールに `month_overview` の書き込み許可を追加（削除トランザクションの失敗を回避）
  - 会議室判定を「表記ゆれ」に強い方式へ変更（`roomName`に`会議室`が含まれればOK）
  - 未ログイン時はパスコード削除UIを表示しないよう制御

### 🧭 影響範囲
- 既存の削除フローは維持（UI/操作は変えずに安定化）
- `month_overview` は現状未使用のため運用影響なし

---

## [2.1.3] - 2025-11-03 ✨ **UX改善**

### 改善内容
- **複数時限選択時の自動入力**: 台帳ビューから予約枠をクリックして複数時限モードに切り替えた際、クリックした時限が自動的に開始時限に設定されるようになりました
- **予約削除/更新の安定性向上**: リロード方式に変更し、確実に最新状態が反映されるように改善

### 技術的な変更
- `PeriodRangeSelector.tsx`: 複数時限モード切り替え時に`selectedPeriod`を`startPeriod`に自動設定するuseEffectを追加
- `ReservationModal.tsx`: 削除・更新後に300msのディレイ後にリロード
- `MainApp.tsx`: URLパラメータで日付を保持（リロード後も日付が維持される）
